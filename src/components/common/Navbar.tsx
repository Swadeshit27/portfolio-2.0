                ...
                <ul className='flex uppercase max-md:hidden'>
                    {NavItemList.map((item, index) => (
                        <li key={index} className='cursor-pointer mx-3 max-lg:text-sm font-libre relative group'>
                            <Link href={item.path} className={pathname == item.path ? "text-white" : "opacity-60 hover:opacity-100"}>{item.name}</Link>
            ...
            {open && <div className='w-full h-full bg-gray-400/50 uppercase md:hidden fixed top-0  z-50'>
                <div className={`w-4/5  h-full  gradient uppercase  fixed top-0  z-50  flex justify-center items-center ${open ? "translate-x-0  transition-all ease-linear duration-500 " :"-translate-x-[100%]  transition-all ease-linear duration-500"} `}>
                    <div className='w-[98%] h-[99%] bg-gray-800/80 p-6   '>
                        ...
                        <h1 className='mt-6 text-start font-dancing text-2xl'>Swadesh pal</h1>
                        <ul className='  h-[20rem] mx-auto pt-12'>
                            {NavItemList.map((item, index) => (
                                <li key={index} className={`cursor-pointer  font-libre my-3 text- ${pathname === item.path && "text-green-500"} `}>
                                    <Link href={item.path} className='flex items-center gap-x-2' onClick={() => setOpen(false)}>
