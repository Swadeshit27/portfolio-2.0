...
const AcademicDetails = async () => {
    ...
    return (
        <>
            <Subheading subtitle="Academic Details" className=" text-3xl md:text-4xl font-semibold text-rose-500  capitalize text-center mb-12" />
                ...
                <div className="h-[45rem] w-[10px] bg-white mx-auto absolute max-lg:left-8 lg:left-[50%] -z-10 rounded-full"></div>
                {
                    ...
                    educations.map((item, i) => {
                        ...
                        return (
                            <div
                                key={i}
                                className={`flex justify-end min-h-[12rem] lg:w-1/2  pt-12    -translate-x-0.5 gap-x-4 ${i % 2 == 0 ? " max-lg:flex-row-reverse lg:translate-x-11" : " lg:-translate-x-9 lg:ms-auto  flex-row-reverse"
                                    }`}
                            >
                                <EducationCardAnimation isReverse={i % 2 == 0}>
                                    <div
                                        className={`absolute  top-8 border-solid   border-y-transparent border-y-8 ${i % 2 == 0
